...
@Model.register("no-answer-squad")
class ModelSQUAD(Model):
    ...
    def forward(self,  # type: ignore
        ...
        if self.training:
            # sample training
            categorical_probs = Variable(torch.Tensor([0.4, 0.2, 0.2, 0.2]).unsqueeze(0).expand(batch_size, 4)).cuda(cuda_device)
            num_passage /= 2
        ...
        embedded_question = embedded_question.unsqueeze(0).expand(num_passage, -1, -1, -1).contiguous().view(-1, question_length, embedding_dim) 
        # Shape: (batch_size, question_length)
        question_mask = util.get_text_field_mask(question).float()
        # Shape: (batch_size*num_passage, question_length)
        question_mask = question_mask.unsqueeze(0).expand(num_passage, -1, -1).contiguous().view(-1, question_length)
        ...
        # Shape: (batch_size*num_passage, passage_length, passage_length)
        span_start_logits_tiled = span_start_logits.unsqueeze(1).expand(-1, passage_length, passage_length)
        span_end_logits_tiled = span_end_logits.unsqueeze(-1).expand(-1, passage_length, passage_length)
        # Shape: (batch_size*num_passage, passage_length**2)
        span_logits = (span_start_logits_tiled + span_end_logits_tiled).view(-1, passage_length**2)
        # Shape: (batch_size*num_passage, passage_length**2) 
        answer_mask = torch.bmm(passage_mask.unsqueeze(-1), passage_mask.unsqueeze(1)).view(-1, passage_length**2)
        # Shape: (batch_size*num_passage, 1)
        no_answer_mask = Variable(torch.ones(batch_size*num_passage).unsqueeze(-1)).cuda(cuda_device)
        # Shape: (batch_size*num_passage, passage_length**2)
        ...
        if span_start is not None:
            if self.training:
                ...
                span_start = span_start.expand(-1, num_passage).contiguous().view(-1)
                span_end = span_end.expand(-1, num_passage).contiguous().view(-1)
                # Shape: (batch_size*num_passage)
                ...
                loss = nll_loss(util.masked_log_softmax(all_logits, combined_mask), span_target)
