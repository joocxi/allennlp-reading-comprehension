...
@Model.register("model-msmarcov20-sharenorm-sumobj")
    ...
    def forward(self,  # type: ignore
        ...
        if span_start is not None:
            ...
            for b in range(batch_size):
                start_idxs = Variable(torch.LongTensor(range(num_passage*passage_length))).cuda()
                end_idxs = Variable(torch.LongTensor(range(num_passage*passage_length))).cuda()
                for i in range(1, num_spans):
                    if span_start[b, i].data[0] >= 0:
                        ...
                        break
                modified_start_logits = Variable(torch.zeros(num_passage*passage_length)).cuda()
                modified_end_logits = Variable(torch.zeros(num_passage*passage_length)).cuda()
